...
const VoucherMakanan = () => {
    ...
    const fetchService = async () => {
      try {
        ...
        const found = response.data.data.find(
          (item) => item.service_code === "VOUCHER_MAKANAN"
        );
        if (found) {
          setService(found);
          setAmount(Number(found.service_tariff));
        } else {
          console.warn("Service Voucher Makanan tidak ditemukan");
        }
      } catch (error) {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/transaction`,
        {
          ...
          service_code: service?.service_code || "VOUCHER_MAKANAN",
        },
        getToken()
      );
  ...
  if (!service) {
    return (
      <div className="p-6">
        <p>Memuat layanan Voucher Makanan...</p>
      </div>
    );
  }
    ...
    <div className="p-6">
      ...
      <div className="mt-8 mx-auto text-center">
        ...
        >
          ...
          <p>
            Pembelian Voucher Makanan sebesar{" "}
            <strong>
