...
const PaketData = () => {
  ...
  useEffect(() => {
    const fetchService = async () => {
      try {
        ...
        const paketService = response.data.data.find(
          (item) => item.service_code === "PAKET_DATA"
        );
        if (paketService) {
          setService(paketService);
          setAmount(Number(paketService.service_tariff));
        } else {
          console.warn("Service Paket Data tidak ditemukan");
        }
      } catch (error) {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/transaction`,
        {
          ...
          service_code: service?.service_code || "PAKET_DATA",
        },
        getToken()
      );
  ...
  if (!service) {
    return (
      <div className="p-6">
        <p>Memuat data layanan Paket Data...</p>
      </div>
    );
  }
    ...
    <div className="p-6">
      ...
      <div className="mt-8 mx-auto text-center">
        ...
        >
          ...
          <p className="text-lg">Pembayaran Paket Data sebesar </p>
          <p className="text-xl">
            <strong>
