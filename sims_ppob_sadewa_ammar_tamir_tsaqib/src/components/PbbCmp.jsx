...
const PbbCmp = () => {
  ...
  useEffect(() => {
    const fetchPBBService = async () => {
      try {
        const response = await axios.get(`${apiUrl}/services`, getToken());
        const pbbService = response.data.data.find(
          (item) => item.service_code === "PAJAK"
        );
        if (pbbService) {
          setService(pbbService);
          setAmount(Number(pbbService.service_tariff));
        } else {
          console.warn("Service PAJAK tidak ditemukan");
        }
      } catch (error) {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/transaction`,
        {
          ...
          service_code: service?.service_code || "PAJAK",
        },
        getToken()
      );
  ...
  if (!service) {
    return (
      <div className="p-6">
        <p>Memuat data layanan Pajak PBB...</p>
      </div>
    );
  }
    ...
    <div className="p-6">
      ...
      <div className="mt-8 mx-auto text-center">
        ...
        >
          ...
          <p className="text-lg">Pembayaran Pajak PBB sebesar </p>
          <p className="text-xl">
            <strong>
