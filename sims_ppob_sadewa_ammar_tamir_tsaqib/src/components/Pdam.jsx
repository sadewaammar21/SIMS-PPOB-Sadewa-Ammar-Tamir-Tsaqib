...
const Pdam = () => {
  ...
  useEffect(() => {
    const fetchPDAMService = async () => {
      try {
        const response = await axios.get(`${apiUrl}/services`, getToken());
        const pdamService = response.data.data.find(
          (item) => item.service_code === "PDAM"
        );
        if (pdamService) {
          setService(pdamService);
          setAmount(Number(pdamService.service_tariff));
        } else {
          console.warn("Service PDAM tidak ditemukan");
        }
      } catch (error) {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/transaction`,
        {
          ...
          service_code: service?.service_code || "PDAM",
        },
        getToken()
      );
    ...
    <div className="p-6">
      ...
      <div className="mt-8 mx-auto text-center">
        ...
        >
          ...
          <p className="text-lg">Pembayaran PDAM sebesar </p>
          <p className="text-xl">
            <strong>
