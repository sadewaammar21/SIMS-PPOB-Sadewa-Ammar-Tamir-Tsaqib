...
const TV = () => {
  ...
  useEffect(() => {
    const fetchTVService = async () => {
      try {
        const response = await axios.get(`${apiUrl}/services`, getToken());
        const tvService = response.data.data.find(
          (item) => item.service_code === "TV"
        );
        if (tvService) {
          setService(tvService);
          setAmount(Number(tvService.service_tariff));
        } else {
          console.warn("Service TV tidak ditemukan");
        }
      } catch (error) {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/transaction`,
        {
          ...
          service_code: service?.service_code || "TV",
        },
        getToken()
      );
  ...
  if (!service) {
    return (
      <div className="p-6">
        <p>Memuat data layanan TV Berlangganan...</p>
      </div>
    );
  }
    ...
    <div className="p-6">
      ...
      <div className="mt-8 mx-auto text-center">
        ...
        >
          ...
          <p className="text-lg">Pembayaran TV sebesar </p>
          <p className="text-xl">
            <strong>
