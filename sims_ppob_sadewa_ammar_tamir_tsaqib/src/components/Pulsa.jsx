...
const Pulsa = () => {
  ...
  useEffect(() => {
    const fetchPulsaService = async () => {
      try {
        const response = await axios.get(`${apiUrl}/services`, getToken());
        const pulsaService = response.data.data.find(
          (item) => item.service_code === "PULSA"
        );
        if (pulsaService) {
          setService(pulsaService);
          setAmount(Number(pulsaService.service_tariff));
        } else {
          console.warn("Service PULSA tidak ditemukan");
        }
      } catch (error) {
  ...
  const handleConfirmPulsa = async () => {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/transaction`,
        {
          ...
          service_code: service?.service_code || "PULSA",
        },
        getToken()
      );
    ...
    <div className="p-6">
      ...
      <div className="mt-8 mx-auto text-center">
        ...
        >
          ...
          <p className="text-lg">Pembelian Pulsa sebesar </p>
          <p className="text-xl">
            <strong>
