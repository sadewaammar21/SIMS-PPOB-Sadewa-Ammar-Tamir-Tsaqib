...
const Listrik = () => {
  ...
  useEffect(() => {
    const fetchPLNService = async () => {
      try {
        const response = await axios.get(`${apiUrl}/services`, getToken());
        const plnService = response.data.data.find(
          (item) => item.service_code === "PLN"
        );
        if (plnService) {
          setService(plnService);
          setAmount(Number(plnService.service_tariff));
        } else {
          console.warn("Service PLN tidak ditemukan");
        }
      } catch (error) {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/transaction`,
        {
          ...
          service_code: service?.service_code || "PLN",
        },
        getToken()
      );
    ...
    <div className="p-6">
      ...
      <div className="mt-8 mx-auto text-center">
        ...
        >
          ...
          <p className="text-lg">Pembayaran Listrik sebesar </p>
          <p className="text-xl">
            <strong>
