...
const Musik = () => {
  ...
  useEffect(() => {
    const fetchMusikService = async () => {
      try {
        const response = await axios.get(`${apiUrl}/services`, getToken());
        const musikService = response.data.data.find(
          (item) => item.service_code === "MUSIK"
        );
        if (musikService) {
          setService(musikService);
          setAmount(Number(musikService.service_tariff));
        } else {
          console.warn("Service MUSIK tidak ditemukan");
        }
      } catch (error) {
  ...
  const handleConfirmMusik = async () => {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/transaction`,
        {
          ...
          service_code: service?.service_code || "MUSIK",
        },
        getToken()
      );
  ...
  if (!service) {
    return (
      <div className="p-6">
        <p>Memuat data layanan Musik Berlangganan...</p>
      </div>
    );
  }
    ...
    <div className="p-6">
      ...
      <div className="mt-8 mx-auto text-center">
        ...
        >
          ...
          <p className="text-lg">Pembayaran Musik sebesar </p>
          <p className="text-xl">
            <strong>
