    ...
    const fetchService = async () => {
      try {
        ...
        const found = response.data.data.find(
          (item) => item.service_code === "QURBAN"
        );
        if (found) {
          setService(found);
          setAmount(Number(found.service_tariff));
        } else {
          console.warn("Service Qurban tidak ditemukan");
        }
      } catch (error) {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/transaction`,
        {
          ...
          service_code: service?.service_code || "QURBAN",
        },
        getToken()
      );
